<template>
  ...
  " :style="getDrawInfo(condition)"><span>

                {{ index }}
                {{ getRangeText(condition)
                }}</span>
        </div>
    </div>

    <div v-if="thereIsSecondPart" style="margin-bottom: 0.25rem; position: absolute;"
...
const thereIsSecondPart = computed(() => !isArrivalBeforeDeparture(departureSeconds, arrivalSeconds) && departureSeconds != arrivalSeconds)
...
function getDrawInfo() {
    ...
    if (isArrivalBeforeDeparture(departureSeconds, arrivalSeconds)) {
        ...
        style.background = 'linear-gradient(to right,  red ' + left + '%, transparent ' + left + '%, transparent ' + right + '%, red ' + right + '%)'
    }
    return style
}
